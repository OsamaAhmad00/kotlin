...
class WasmIrSerializerImpl(val outputStream: OutputStream) : WasmIrSerializer {
     ...
     *
     * See [WasmIrDeserializerImpl]
     */

    private val b: ByteWriter = ByteWriter.OutputStream(outputStream)

    override fun serialize(func: WasmFunction) {
    ...
    override fun serialize(global: WasmGlobal) =
    ...
    override fun serialize(funcType: WasmFunctionType) =
        serializeNamedModuleField(funcType) {
            serialize(funcType.parameterTypes)
            serialize(funcType.resultTypes)
        }

    override fun serialize(typeDecl: WasmTypeDeclaration) =
        when (typeDecl) {
    ...
    private fun serialize(i: WasmImmediate) =
        when (i) {
    ...
    private fun serialize(value: WasmTable.Value) =
        when (value) {
    ...
    private fun serialize(element: WasmElement) =
    ...
    private fun serialize(list: List<Any>) {
